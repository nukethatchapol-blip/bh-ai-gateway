...
export function parseAnthropicEvent(line) {
  ...
  if (json.type === "content_block_delta" && json.delta?.type === "thinking_delta") {
    return { type: "thinking-delta", text: json.delta.thinking };
  }
...
async function pumpSSE(response, parseLine, onEvent) {
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      else if (ev.type === "thinking-delta") { onEvent(ev); }
    }
  }
  return { text };
}
  ...
  if (m.provider === "anthropic") {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 4096,
        // Extended thinking: the model's reasoning streams as separate
        // thinking_delta events, kept out of the final answer text.
